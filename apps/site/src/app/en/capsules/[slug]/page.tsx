...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${SITE_URL}/en/capsules/${capsule.slug}`,
      languages: {
    ...
    openGraph: {
      ...
      url: `${SITE_URL}/en/capsules/${capsule.slug}`,
      type: "article",
      locale: "en_US",
    },
  };
...
export default async function CapsuleEnPage({ params }: PageProps) {
  ...
  // JSON-LD: CollectionPage with hasPart → ItemList
  const jsonLd = {
      ...
      <main className="bg-vz-bg">
        <section className="max-w-4xl mx-auto px-6 py-12 lg:py-16">
          ...
          <p className="text-xs uppercase tracking-[0.14em] text-vz-ink-mute mb-3">
            {new Date(capsule.published_at).toLocaleDateString("en-GB", {
              year: "numeric",
              month: "long",
            })}{" "}
