  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <main className="md:ml-64 p-4 md:p-8 max-w-5xl">
        <Link href="/ia" className="inline-flex items-center gap-2 text-sm text-teal hover:text-teal-700 mb-4">
        ...
        <div className="rounded-3xl bg-gradient-signature p-6 md:p-8 text-white shadow-depth mb-6 relative overflow-hidden">
              ...
              <Card title="Actions prioritaires" icon={<span>🚀</span>}>
                <ol className="space-y-3">
                  {r.actions_prioritaires.map((a: any, i: number) => (
                    <li key={i} className="flex items-start gap-3 rounded-xl bg-white border border-teal-50 px-4 py-3">
                      <span className="h-7 w-7 rounded-full bg-teal text-white flex items-center justify-center text-sm font-semibold flex-shrink-0">
                        {i + 1}
                      </span>
                      <div className="min-w-0">
                        ...
                        {a.priorite && (
                          <span className="inline-block mt-1 text-[10px] font-semibold tracking-wider px-2 py-0.5 rounded-full bg-pink-100 text-pink-700 uppercase">
                            {a.priorite}
                          </span>
                        )}
                      </div>
              ...
              <Card title="KPI cibles" icon={<span>📊</span>}>
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  {Object.entries(r.kpi_cibles).map(([k, v]) => (
                    <div key={k} className="rounded-xl bg-cream px-4 py-3">
