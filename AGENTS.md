...
| IA | Anthropic Codex (Codex-haiku-4-5) |
...
POST   /api/seo/social-posts/regenerate      Régénérer la sélection (Codex + fallback)
...
POST   /api/seo/reviews/{id}/suggest-reply   Brouillon de réponse Codex (avec fallback)
...
GET    /api/crm/clients/{id}/personal-shopper-v2  Personal shopper (embeddings + Codex Haiku, manager)
...
### 3. IA Booster
- **Analyse photo** : Codex Vision détecte type, couleur, marque, état, **genre** (H/F/E/U)
...
- **Personal Shopper gated** : réservé aux membres avec consent profilage.
  Recherche sémantique texte libre (« t-shirt blanc taille M ») — Codex Haiku
...
Branches `main` (prod) + branches feature `Codex/<sujet>-<suffix>`. Le push sur
`main` déclenche la GitHub Action `Deploy Production` → SSH → `scripts/deploy.sh`
sur le VPS (`/opt/vintiz`).

```bash
# Cycle type
git checkout -b Codex/feature-xyz
# ... commits ...
git push -u origin Codex/feature-xyz
